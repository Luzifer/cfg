...
def yes_no_question(question):
    while True:
        ...
        if text.lower().startswith("y"):
            ...
            console_print(
                u"Sorry, I didn't understand that. Please type yes or no.")
        ...
        class DownloadDialog(gtk.Dialog):
            ...
            def label_motion(self, widget, event):
                ...
                if index >= link_index and index < link_index+len(LINK):
                    self.hovering = True
                    self.label_box.window.set_cursor(
            ...
            def __init__(self):
                ...
                self.label_box.connect(
                    "motion-notify-event", self.label_motion)
    ...
    def download():
        ...
        def setprogress(text, frac):
            ...
            if sys.stdout.isatty():
                ...
                write(unsave)
            console_print(text % int(100*frac),
...
def command(meth):
    ...
    meth_aliases = [unicode(alias) for alias in aliases.iterkeys(
...
def start_dropbox():
    db_path = os.path.expanduser(
        u"~/.dropbox-dist/dropboxd").encode(sys.getfilesystemencoding())
...
def columnize(list, display_list=None, display_width=None):
    ...
    non_unicode = [i for i in range(len(list)) if not (
        isinstance(list[i], unicode))]
    if non_unicode:
        ...
        with closing(DropboxCommand()) as dc:
            if options.list:
                ...
                if len(args) == 0:
                    ...
                    for a in args:
                        try:
                            (dirs if os.path.isdir(a) else nondirs).append(
                                a.decode(enc))
                ...
                def path_to_string(file_path):
                    if not os.path.exists(file_path):
                        path = u"%s (File doesn't exist!)" % os.path.basename(
                            file_path)
                        return (path, path)
                    try:
                        status = dc.icon_overlay_file_status(
                            path=file_path).get(u'status', [None])[0]
                    ...
                    for subname in sorted(os.listdir(name), key=methodcaller('lower')):
                        ...
                        try:
                            clean, formatted = path_to_string(
                                unicode_abspath(os.path.join(name, subname)))
                ...
                if len(args) == 0:
                    args = [name for name in sorted(os.listdir(
                        u"."), key=methodcaller('lower')) if type(name) == unicode]
                if len(args) == 0:
                ...
                for file in args:
                    ...
                    try:
                        status = dc.icon_overlay_file_status(
                            path=fp).get(u'status', [u'unknown'])[0]
        ...
        with closing(DropboxCommand()) as dc:
            try:
                path = unicode_abspath(
                    args[0].decode(sys.getfilesystemencoding()))
                link = dc.get_shared_link(path=path).get(
                    'link', [u'No link'])[0]
                console_print(link)
            except DropboxCommand.CommandError, e:
...
"""
    ...
    kwargs = dict(
        zip(ARGS, [a.decode(sys.getfilesystemencoding()) for a in args]))
...
"""
    ...
    if should_lansync is None:
        ...
        with closing(DropboxCommand()) as dc:
            dc.set_lan_sync(
                lansync='enabled' if should_lansync else 'disabled')
    ...
    if len(args) == 0:
        try:
            with closing(DropboxCommand()) as dc:
                try:
                    lines = [relpath(path)
                             for path in dc.get_ignore_set()[u'ignore_set']]
                    lines.sort()
                    if len(lines) == 0:
                    ...
                    console_print(
                        u"Couldn't get ignore set: daemon isn't responding")
                except DropboxCommand.CommandError, e:
                    if e.args[0].startswith(u"No command exists by that name"):
                        console_print(
                            u"This version of the client does not support this command.")
                    else:
        ...
        sub_command = args[0]
        paths = args[1:]
        absolute_paths = [unicode_abspath(path.decode(
            sys.getfilesystemencoding())) for path in paths]
                ...
                with closing(DropboxCommand(timeout=None)) as dc:
                    try:
                        ...
                        if result[u"ignored"]:
                            console_print(u"Excluded: ")
                            lines = [relpath(path)
                                     for path in result[u"ignored"]]
                            for line in lines:
                                console_print(unicode(line))
                    except KeyError:
                        console_print(
                            u"Couldn't add ignore path: daemon isn't responding")
                    except DropboxCommand.CommandError, e:
                        if e.args[0].startswith(u"No command exists by that name"):
                            console_print(
                                u"This version of the client does not support this command.")
                            ...
                            console_print(
                                u"Couldn't get ignore set: " + str(e))
                    except DropboxCommand.BadConnectionError, e:
                ...
                with closing(DropboxCommand(timeout=None)) as dc:
                    try:
                        ...
                        if result[u"removed"]:
                            ...
                            lines = [relpath(path)
                                     for path in result[u"removed"]]
                            for line in lines:
                                console_print(unicode(line))
                    except KeyError:
                        console_print(
                            u"Couldn't remove ignore path: daemon isn't responding")
                    except DropboxCommand.CommandError, e:
                        if e.args[0].startswith(u"No command exists by that name"):
                            console_print(
                                u"This version of the client does not support this command.")
